...
| 角色 | 核心特征 | 本 PRD 能力边界 |
|---|---|---|
| **学生（妹妹）** | 13–16 岁，初中在读，使用**北师大版**数学教材（七年级起为 2024 新版），目标考区为**贵阳中考**（数学满分 150）；遇到不会的题习惯拍照求助。**学龄**：MVP 阶段全局可配置（env `STUDENT_GRADE_TERM`，当前=初一下），引导知识边界以此为准（见 §8.1） | 可注册登录、拍题 / 选图、与 Agent 多轮对话、查看历史会话；**不能调整 Agent 参数**、**不能跨学科提问** |
...
- **知识边界（学龄，2026-06-05 新增）**：解题路线设计（VisionNode 拆题 milestones）与全程引导**只能使用学生已学知识**——以全局配置的学龄（env `STUDENT_GRADE_TERM`，当前=初一下）按北师大版教材目录截断；当前就读学期视为「正在学习中」，优先使用更早学期知识。**生成端硬约束、学生端宽容**：Agent 自己严禁主动引入未学概念；但学生**主动**提出超纲方法且使用正确时，顺势肯定并点出纲内等价做法，不强行拉回。教材体系以**北师大版**为唯一基准（模型默认的人教版章节体系视为错误，实测案例：会话 ca6e1b83 对初一下学生引导了八下的垂直平分线判定定理，纲内做法应为七下的全等+三线合一）。
