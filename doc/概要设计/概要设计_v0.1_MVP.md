...
| 版本 | v0.1 + v0.1.1 增量补丁（B4 下沉） |
| 状态 | 草稿 |
| 创建日期 | 2026-05-19 |
| 最后更新 | 2026-05-21（v0.1.1 增量补丁：OcrSchema 多题目标态 + B4 state subProblems / 双层循环路由 / 2 个 SSE chunk；DR-010 闭环 + AR-012 概要侧闭环；新增 C14 / C15 / C16 决策） |
| 编写人 | Claude（开发主力）|
...
| 业务 | `metadata` | **新增** `jsonb` | NOT NULL | `'{}'::jsonb` | **语义化业务结构数据**；MVP 已知 key：`knowledgeCard`（P-105，详见 §4.3）；与公共字段 `ext_info` 区别详见 §3.2.1 注 |
...
### 4.1 ElicitGraphStateSchema 完整定义（v0.1.1 目标版）

> 现状见 `src/agents/schemas/ElicitGraphStateSchema.ts:8-18`（6 字段）。
> v0.1 新增 5 字段：`currentPhase` / `problemType` / `stuckCountPerPhase` / `probedQuestionIds` / `lastDeviationAt`。
> **v0.1.1 补丁（B4 下沉，2026-05-21）**：① 新增 `subProblems[]` / `currentSubProblemIndex` 承载外层多小问推进 + 内层多破题点循环（FR-AGENT-008/009 / AR-012）；② `stuckCountPerPhase` / `probedQuestionIds` 从顶级**下沉**到 `subProblems[i]` 子字段（per-subProblem 隔离，避免跨小问污染）；③ `OcrSchema` 改为多题数组目标态（详见 §4.2）。完整决策见 §10 C14 / C15 / C16。

```ts
// src/agents/schemas/ElicitGraphStateSchema.ts（v0.1.1 目标版）
import { z } from "zod";
...
// 卡壳计数（per phase + per subProblem 隔离）
...
// SubProblem 运行时状态（B4 下沉新增；FR-AGENT-008/009 + AR-012）
// ⚠ 注：与 OcrSchema.questions[i].subProblems 同名但语义不同——
//   · OcrSchema.questions[i].subProblems = VisionNode 静态拆分（题目结构）
//   · state.subProblems[i] = 运行时态（status / insightPoints / per-subProblem 计数）
// ClassifyNode 在 /resolve 触发的二次 invoke 中从 ocrResult.questions[selectedQuestionIndex].subProblems 拷入静态字段 + 填默认 runtime
const SubProblemStateSchema = z.object({
    // —— 静态部分（从 OcrSchema 拷入）——
    index:           z.number().int().min(0),
    goal:            z.string().min(1).max(50),       // 例 "求 m 的取值范围"
    ...
    insightPoints:      z.array(z.string()).default([]),           // 妹妹说出的破题点列表（FR-AGENT-009）
    stuckCountPerPhase: StuckCountSchema.default(() => ({
        understand: 0, plan: 0, execute: 0, review: 0,
    })),
    probedQuestionIds:  z.array(z.number().int().min(1).max(5)).default(() => []),  // 5 问探路已用
});
...
export const ElicitGraphStateSchema = z.object({
    ...
    // —— v0.1 新增（3 字段；原 5 字段中 stuckCountPerPhase + probedQuestionIds 下沉到 SubProblemStateSchema）——
    ...
    // —— v0.1.1 B4 下沉新增 2 字段（FR-AGENT-008/009 + AR-012；C15 / C16）——
    subProblems:             z.array(SubProblemStateSchema).default(() => []),
        // 多小问运行时态；单问题题 length=1 统一路径；ClassifyNode 在 /resolve 二次 invoke 中初始化
    currentSubProblemIndex:  z.number().int().min(0).default(0),
        // 外层游标；由 §7.1 / §8.3 `subProblemRouter` 维护
});
...
> **v0.1.1 B4 下沉的命名约定与不变量**（C15 + C16 决策详见 §10）：
> - `state.subProblems` 与 `OcrSchema.questions[i].subProblems` **同名但语义不同**：前者运行时，后者静态拆分；命名一致是为了让"小问"概念在产品 / 接口 / state 三层用同一个词（沿用 PRD §8 用语表）
> - **单问题题统一路径**：单一目标的题视为 `state.subProblems.length === 1` 的特例，无任何 if-else 分支（PRD §8.2 外层段 / 需求 FR-AGENT-008）
> - **外层游标推进**：`currentSubProblemIndex` 由 §7.1 `subProblemRouter` 在收到 `SUB_PROBLEM_DONE` / `PROBLEM_BLOCKED` 信号时 +1，越界即进入阶段 ④
> - **内层循环**：`currentSubProblemIndex` 不变，仅 `currentPhase` 在 PLAN ↔ EXECUTE 间来回，每轮可往 `subProblems[currentSubProblemIndex].insightPoints` 追加破题点（FR-AGENT-009 内层 / 详设 §15 D12）
> - **per-subProblem 隔离不变量**：`stuckCountPerPhase` 与 `probedQuestionIds` 跟随 subProblem 走，切换小问时不带入旧计数（防止小问 (2) 入场时计数已满导致 StuckGuard 误触发）
...
### 4.2 OcrSchema 多题契约（v0.1.1 B4 下沉新增）

> **v0.1 现状（POC `src/agents/schemas/OcrSchema.ts`）**：单数 `sanitizedContent` 字段、仅支持单题。
> **v0.1.1 目标态（B4 下沉，2026-05-21）**：①改为 `questions[]` 数组（1~5）+ `selectedQuestionIndex?` + `isMulti`；②每个 `SanitizedQuestion` 内嵌 `subProblems[]` 静态拆分（VisionNode 一次性输出，FR-VISION-001）；③`discriminatedUnion` on `isSolvable`。完整决策见 §10 C14（OcrSchema 目标态）。
>
> **闭环风险**：DR-010（详设 §4.3.1 / §9 / §13.4 / §15 D10 已按目标态写；本节回补让 OcrSchema.parse 与 prompt 输出契合）。

#### 4.2.1 Zod 完整定义

```ts
// src/agents/schemas/OcrSchema.ts（v0.1.1 目标版；权威实现以源码为准）
import { z } from "zod";

// 小问静态拆分（VisionNode 输出；ClassifyNode 在 /resolve 二次 invoke 中拷入 state.subProblems）
...
// 单题结构（VisionNode 识别出的一道题）
const SanitizedQuestionSchema = z.object({
    index:              z.number().int().min(0).max(4),          // 0-based 序号；与 questions 数组索引一致
    topic:              z.string().min(1).max(30),
    latexFull:          z.string(),                              // 完整 LaTeX 题面
    givenConditions:    z.array(z.string()).default([]),
    implicitConditions: z.array(z.string()).default([]),
    goal:               z.string().min(1).max(30),
    milestones:         z.array(z.string()).max(4).default([]),

    // R-011 落地（详设 §4.3.1）：本题是否依赖图 + DeepSeek 文本侧消费的视觉描述
    visualFeaturesNeeded: z.boolean().default(false),
    visualDescription:    z.string().default(''),
        // visualFeaturesNeeded === true 时必填 ≥30 字（详设 §4.3.1 prompt checklist 覆盖：
        //   函数图像题 / 几何图形题 / 统计图表题 / 数轴 / 立体图等）；
        // false 时为空字符串；DeepSeek 后续节点（ClassifyNode / PhaseNode）user prompt
        // 通过 formatVisualBlock(selectedQuestion) 注入 `# 题目图示` 段透传给文本模型

    // B4 新增：题目内小问拆分（单问题题 length=1，多小问题 length 2~5）
...
const SolvableOcrSchema = z.object({
    ...
    subject:               z.literal('math'),                                  // 仅 math 进下游
    grade:                 z.enum(['小学', '初中', '高中', '其他']).nullable(),
    questions:             z.array(SanitizedQuestionSchema).min(1).max(5),
    selectedQuestionIndex: z.number().int().min(0).max(4).optional(),          // /resolve 写回；多题前端选；单题自动 0
    isMulti:               z.boolean(),                                        // = (questions.length >= 2)
    visualFeaturesNeeded:  z.boolean().default(false),                         // 任一题含函数图像 / 统计图表
...
const UnsolvableOcrSchema = z.object({
    ...
    subject:     z.string(),                                                   // 实际学科（≠ math）
    questions:   z.array(SanitizedQuestionSchema).length(0),                   // 空数组（保留 discriminatedUnion 字段对齐）
...
| 场景 | VisionNode 输出 | SSE chunk | 前端表现 | `/resolve` 写回 |
|---|---|---|---|---|
| **单题** | `questions.length === 1`，`isMulti === false` | `questions_detected`（§9.1） | **不弹** P-103，自动调 `/resolve` 不带 `selectedQuestionIndex`（隐式 = 0） | `selectedQuestionIndex = 0` |
| **多题** | `questions.length 2~5`，`isMulti === true` | 同上 | **弹** P-103 浮层让妹妹选 | `selectedQuestionIndex = k`（妹妹选定） |
| **不可解** | `isSolvable === false`，`questions === []` | 不推 `questions_detected`（推 `vision_error`，详设 §8.3） | 错误界面（详设 §8.3 话术表） | — |

#### 4.2.3 与现状差异 / 实施期硬门控

- POC 现状 `OcrSchema` 单数 `sanitizedContent` → 实施期一次性重写为目标态（不向后兼容；POC 数据库无历史 OcrSchema 写入，零迁移成本）
- 详设 §4.3.1 VisionNode prompt 已按目标态写；详设 §13.4 时序图 / §15 D10 / §18 准入第 19 项均依赖本节
- 详设 §18 准入第 19 项硬门控：**先回补本节再落 VisionNode 代码**；否则 OcrSchema.parse 会与 prompt 输出 questions[] 撞（DR-010 闭环条件）

---

### 4.3 P-105 卡片 inline JSON Schema
...
> v0.1.1 B4 下沉：① 新增 `subProblems[]` / `currentSubProblemIndex` 行；② 原顶级 `stuckCountPerPhase` / `probedQuestionIds` 改为 `subProblems[i].*` 子字段（标注为 `RW (→ sub[i])`）。
...
| `subProblems[]` | — | — | — | W（初始化）| R | RW（写 insightPoints） | RW（写 insightPoints / status）| RW（读全部、改 status） | R（SSE `sub_problem_changed` 同步顶栏）|
| `currentSubProblemIndex` | — | — | — | W（= 0）| R | R | R（由 subProblemRouter 在 SUB_PROBLEM_DONE / PROBLEM_BLOCKED 后 +1）| R | R（SSE `sub_problem_changed`）|
| `subProblems[i].stuckCountPerPhase` | — | — | — | — | RW | RW | RW | RW | — |
| `subProblems[i].probedQuestionIds` | — | — | — | — | RW | RW | RW | — | — |
...
export const shouldClassify:           (state: ElicitGraphState) => string[];
    // (state.hasResolved && !state.problemType) → [classifyNodeName] | []；详见 §8.3
export const phaseRouter:              (state: ElicitGraphState) => string;
    // 历史会话恢复入口；按 state.currentPhase 派发到对应 Pólya 阶段节点

// —— v0.1.1 B4 下沉新增 2 条路由（FR-AGENT-008/009；详设 §15 D12）——
export const subProblemRouter:         (state: ElicitGraphState) => string;
    // 外层多小问推进；ExecuteNode 出口在 SUB_PROBLEM_DONE / PROBLEM_BLOCKED 信号后调用：
    //   · 若 state.currentSubProblemIndex + 1 < state.subProblems.length
    //     → 把游标 +1 + 当前阶段重置到 UNDERSTAND，返回 understandNodeName（下一小问从阶段 ① 重新进入）
    //   · 否则（所有小问 ∈ {done, blocked}）→ 返回 reviewNodeName（进阶段 ④ 1 张大卡片）
    //   · 单问题题（subProblems.length === 1）路径同样适用，第一次完成即直接进 reviewNodeName

export const insightLoopRouter:        (state: ElicitGraphState) => string;
    // 内层 PLAN ↔ EXECUTE 循环；ExecuteNode 出口在 ESCALATE 信号后调用：
    //   · 妹妹明确想换方向 → 返回 planNodeName（回阶段 ② 拟定下一个破题点）
    //   · 否则保留在 executeNodeName（STAY 信号语义）
```
...
> **v0.1.1 B4 下沉重写**：原 v0.1 输出 `sanitizedContent` 单数 + 单题自动写 `hasResolved=true` 的契约，与 PRD F002 / US-004 / FR-VISION-002 多题流程冲突，由详设 §15 D10 修正、本节回补。完整 Zod 见 §4.2。

- **输入**：`state.questionImgUrl`（必需）+ `state.messages` 末位用户文本（可选辅助说明）
- **输出**：`{ ocrResult: OcrSchema, questionImgUrl?: undefined }`
  - `ocrResult.isSolvable === true`：输出 `questions[]`（1~5，每题含 `subProblems[]` 静态拆分）+ `isMulti` + `subject = 'math'` + `selectedQuestionIndex = undefined`（等 `/resolve` 写回）+ `visualFeaturesNeeded`
  - `ocrResult.isSolvable === false`：`errorReason` 填入 `BLURRY` / `NOT_SOLVABLE` / `TIMEOUT` / `PARSE_FAIL`；前端由 §8.3 错误话术映射展示（详设 §8.3）
  - **不在本节点写 `hasResolved`**：单题 / 多题均由 `/resolve` 路由（§5.4）统一写回（避免单题路径与多题路径行为不一致）；本节点出口仅清 `questionImgUrl`
- **失败模式**：模型调用异常 / 返回不符合 schema → 由 `visionFailureGuard` 在下一节点入口拦截（详细设计 §5.6 / D2）
- **SSE 副作用**：本节点在出口前 `getWriter()` 推送 `questions_detected` chunk（§9.1）让前端知晓题目数（多题弹 P-103；单题自动调 `/resolve`）

> **B4 静态拆分**：VisionNode 在同一次 LLM 调用中输出每题的 `subProblems[]`（FR-VISION-001 / 详设 §15 D13 VisionNode 集成 Decomposition，不增独立 DecompositionNode）；ClassifyNode 在 `/resolve` 二次 invoke 中根据 `selectedQuestionIndex` 拷贝 `ocrResult.questions[idx].subProblems` 到 `state.subProblems[]` 并初始化运行时字段。
...
// —— v0.1.1 B4 下沉新增 2 条路由（详设 §15 D12 双层循环条件边）——
// 注：这两个路由 **不属于 startFanoutNode 的子路由**，而是 ExecuteNode 出口的独立条件边
export const subProblemRouter:         (state) => string;
    // 外层条件边：ExecuteNode → understandNodeName（下一小问） / reviewNodeName（全部完成）
    // 触发：state.subProblems[currentSubProblemIndex].status ∈ {'done', 'blocked'}
    // 副作用：可能写 currentSubProblemIndex += 1 + currentPhase = UNDERSTAND（重置到阶段 ①）
    // SSE：切换小问时推 `sub_problem_changed` chunk（§9.1）

export const insightLoopRouter:        (state) => string;
    // 内层条件边：ExecuteNode → planNodeName（回阶段 ②） / executeNodeName（STAY）
    // 触发：ExecuteNode 输出 phase_signal === 'ESCALATE'
    // 副作用：不改 subProblems 静态字段；可能往 subProblems[currentSubProblemIndex].insightPoints 追加破题点
```
...
- **B4 双层循环不变量**（C16 决策）：① 外层 / 内层路由都不引入新节点（沿用高层 §8 D2 + B4 补充：5 个 Pólya 节点 + 守卫框架内的"条件边"扩展）；② 路由函数纯函数 + 无副作用 / 仅返回节点名；副作用（如 SSE chunk / state 重置）由调用方节点显式做
...
type KnowledgeCardChunk = {
    ...
    data: {
        kind: 'knowledge_card';
        card: KnowledgeCard;     // 见 §4.3
    };
};

// 5. 多题检测（v0.1.1 新增；由 VisionNode 在出口前推送；FR-VISION-002 / 详设 §13.4）
type QuestionsDetectedChunk = {
    id: string;                  // 会话 id 或 message_id
    type: 'data-custom';
    delta: '';
    data: {
        ...
        isMulti: boolean;        // questions.length >= 2
    };
};
// 前端处理：isMulti=true → 弹 P-103 浮层让妹妹选；isMulti=false → 自动调 /resolve（隐式 index=0）

// 6. 小问切换（v0.1.1 B4 下沉新增；由 SubProblemRouter 推进游标时推送；FR-AGENT-006 双层顶栏 / FR-AGENT-008）
type SubProblemChangedChunk = {
    id: string;                  // 会话 id
    type: 'data-custom';
    delta: '';
    data: {
        kind: 'sub_problem_changed';
        from: number;            // 旧 currentSubProblemIndex
        to: number;              // 新 currentSubProblemIndex
        total: number;           // subProblems.length
        // 可选：当前小问的 goal 文本（前端可不存 ocrResult 也能渲染顶栏）
        currentGoal?: string;
    };
};
// 前端处理：P-104 顶栏切换"小问 N/M"显示；单问题题（total === 1）前端不显示"小问 N/M"前缀
```
...
| `data-custom` | `questions_detected` | `isMulti===true` → `setPendingQuestions(questions)` 弹 P-103；`isMulti===false` → `await confirmSelectedQuestion(0)`（自动调 `/resolve`）|
| `data-custom` | `sub_problem_changed` | `set({ currentSubProblemIndex: chunk.data.to })`；P-104 顶栏切换"小问 N/M · 阶段名"；`total === 1` 时不显示"小问 N/M"前缀 |
| `data-custom` | _未知 kind_ | **default 分支静默忽略**（CR-005 / 详设 §15 D7）；旧 tab 升级兼容策略 |
...
| **C14** | OcrSchema 目标态（多题 vs 单数 sanitizedContent） | OcrSchema 改为 `questions[]` 数组（1~5）+ `selectedQuestionIndex?` + `isMulti` + `subProblems[]` 静态拆分；POC 单数形态零迁移成本一次性重写 | DR-010 闭环 / PRD F002 / FR-VISION-002 / 详设 §15 D10 |
| **C15** | B4 多小问 state schema 字段（FR-AGENT-008/009） | state 顶级加 `subProblems[]` (运行时态) + `currentSubProblemIndex`；`stuckCountPerPhase` / `probedQuestionIds` 从顶级下沉到 `subProblems[i]` 子字段；嵌套结构而非扁平 (subProblemIdx, phase) 3D 数组 | AR-012 闭环 / FR-AGENT-008/009 / 详设 §15 D14 |
| **C16** | B4 双层循环条件边形态（外层 subProblem 推进 + 内层 PLAN ↔ EXECUTE） | 加 2 个路由函数 `subProblemRouter` / `insightLoopRouter`，作为 ExecuteNode 出口的独立条件边；**不引入新节点**（沿用高层 §8 D2 + B4 补充 5 个 Pólya 节点 + 守卫框架）；新增 2 个 SSE chunk `questions_detected` + `sub_problem_changed` | AR-012 闭环 / FR-AGENT-006/008/009 / 详设 §15 D12 / D13 |
...
#### C14 OcrSchema 目标态（v0.1.1 B4 下沉新增）

- **问题陈述**：POC 现状 `OcrSchema.sanitizedContent` 单数字段、仅支持单题；与 PRD F002 + US-004 + FR-VISION-002 多题选一明确冲突。详设 §4.3.1 / §13.4 / §15 D10 已按目标态写、概要 §4.2 之前还是单数，落地阶段会发生"prompt 输出 questions[] 但 OcrSchema.parse 失败"（DR-010 高严重度风险）。
- **候选方案**：
  - A：保留 POC 单数 + 由前端拆图二次上传识别下一题 — 违反 PRD §4.3"识别全部 + 选一"，体验差
  - B：OcrSchema 改 `questions[]` 数组 + `selectedQuestionIndex?` + `isMulti` + `subProblems[]` 静态拆分 — 与详设 §4.3.1 prompt 输出契合
  - C：保留 sanitizedContent 单数 + 新增独立 `MultiQuestionsSchema` 平行字段 — 字段冗余、两套 reader
- **采纳**：方案 **B**（详见 §4.2 完整 Zod）
- **理由**：
  - 多题识别是 PRD Must 范围（FR-VISION-002 Must 优先级），不能延迟
  - `questions[]` + `selectedQuestionIndex` 模型与概要 §5.4 `/resolve` 入参天然吻合（设计 §5.4 时就考虑过多题）
  - POC `OcrSchema.ts` 数据库无历史写入（PostgresSaver checkpoint 未持久化目标态），**零迁移成本**重写
  - 与 B4 静态拆分集成：每个 `SanitizedQuestion` 内嵌 `subProblems[]`，由 VisionNode 在同一 prompt 中输出（FR-VISION-001 / 详设 §15 D13）
- **影响范围**：
  - §4.2 新增 OcrSchema 完整定义节
  - §7.4 VisionNode I/O 契约重写（不再写 hasResolved；推 `questions_detected` chunk）
  - §9.1 SSE chunk 新增 `questions_detected` kind
  - §10 决策表新增本条
  - §13.1 准入新增 4a 项
  - **实施期硬门控**：详设 §18 准入第 19 项 — 先回补本节再落 VisionNode 代码
- **备选退路**：若 qwen-vl-max 多题识别准确率 < 80%（详设 §15 D10 备选退路），降级为"单题识别 + 妹妹手动输入'还有别的题'触发二次识别"——属于产品交互降级，不动 OcrSchema 目标态

#### C15 B4 多小问 state schema 字段（v0.1.1 B4 下沉新增）

- **问题陈述**：B4 升级（2026-05-21）引入"外层多小问推进 + 内层多破题点循环"（PRD §8.2 / FR-AGENT-008/009 / AR-012），但 v0.1 state schema 顶级仅 5 字段、无 subProblems 槽位。
- **候选方案**：
  - A：扁平 state 字段 — `subProblemsCount`/`currentSubProblemIndex`/`subProblemStatuses: string[]`/`insightPointsPerSubProblem: string[][]`/`stuckCountPerSubProblemAndPhase: number[][][]` 等多个并行数组（3D 嵌套）
  - B：嵌套 state 字段 — `subProblems: SubProblemStateSchema[]` 每个 item 含 status / insightPoints / stuckCountPerPhase / probedQuestionIds + `currentSubProblemIndex: number`；原顶级 `stuckCountPerPhase` / `probedQuestionIds` 下沉到 SubProblemStateSchema 子字段
  - C：用 LangGraph channel-specific reducers 分通道维护 — 过度抽象
- **采纳**：方案 **B**（嵌套）
- **理由**：
  - 扁平方案 3D 数组同步成本高、单测覆盖困难；嵌套让 SubProblem 成为独立 unit
  - per-subProblem 隔离不变量自然满足：切换小问时整个 SubProblemState 对象切换，不可能漏改 stuckCount
  - 与 OcrSchema.questions[i].subProblems 静态拆分一一对应，命名 / 概念跨层一致
  - LangGraph state 嵌套 zero-overhead（仅序列化体积稍大）
- **影响范围**：
  - §4.1 ElicitGraphStateSchema 重构（新增 SubProblemStateSchema + 顶级 2 字段 + 顶级 2 字段下沉）
  - §4.4 读写矩阵更新
  - §7.4 VisionNode 不再写 hasResolved；ClassifyNode 在 /resolve 二次 invoke 中初始化 state.subProblems
  - 详设 §4.3 各 PhaseNode prompt user template 加 `currentSubProblem` + `insightPoints[]` 上下文（详设 B4 下沉同步）
- **备选退路**：v0.2 若 checkpoint P95 > 100KB（AR-012 mitigation #4），评估 `insightPoints` 持久化形态调整（如改存到 elicit_messages.metadata 而非 state），不改 schema 顶级结构

#### C16 B4 双层循环条件边形态（v0.1.1 B4 下沉新增）

- **问题陈述**：B4 引入外层 SubProblem 推进 + 内层 PLAN ↔ EXECUTE 循环（详设 §15 D12 已记录但未在概要落到接口契约层）。需要确定路由函数形态 + 是否引入新节点。
- **候选方案**：
  - A：引入独立节点 `SubProblemRouterNode` + `InsightLoopRouterNode` — 增加 2 个节点 + 拓扑复杂度
  - B：作为 ExecuteNode 出口的条件边路由函数（纯函数返回节点名）— 沿用高层 §8 D2 + B4 补充的"5 个 Pólya 节点 + 守卫"框架，不增节点
  - C：把双层循环逻辑嵌入 ExecuteNode 函数体 — Node 内部判断 next，违反 LangGraph 显式拓扑原则
- **采纳**：方案 **B**（条件边路由函数）
- **理由**：
  - 高层 §8 D2 + B4 补充明确"不引入新节点 / 不拆 subgraph"，方案 A 违反
  - 路由函数纯函数 + 显式拓扑，调试 / 单测友好；副作用（如 SSE chunk / state 重置）由调用方节点显式做
  - 同时新增 2 个 SSE chunk（`questions_detected` / `sub_problem_changed`）让前端 P-104 双层顶栏（FR-AGENT-006）有数据源
- **影响范围**：
  - §7.1 + §8.3 新增 `subProblemRouter` / `insightLoopRouter` 路由函数签名
  - §9.1 新增 2 个 SSE chunk kind
  - 详设 §15 D12 决策正文（待补 — B4 下沉的一部分）
  - 详设 §13 时序图新增外层 / 内层流转（待补）
- **备选退路**：若实施期发现条件边判断逻辑复杂到 > 50 行（如需多次访问 DB），评估升级到方案 A（拆独立节点）；schema 字段不变
...
| **CR-003** | `metadata jsonb` 演化时缺 schema 版本号 → 旧消息 metadata 与新前端 reader 不兼容 | §4.3 / S2 前端 | 中 | `KnowledgeCardSchema.schemaVersion = 1` 已加；前端 reader 在 `card.schemaVersion !== 1` 时 fallback 纯文本 | 详细设计 D2（前端 reader 实现） |
...
| **CR-007** | `ext_info`（公共预留）与 `metadata`（业务语义化）混用 → 卡片数据被塞进 `ext_info` 或反之 | §3.2.1 / §4.3 / S5 ReviewNode | 低 | §3.2.1 注表 + 模版规约 + PR review；Mapper.insertAssistant 仅接受 `metadata?: MessageMetadata`，不暴露 `extInfo` 入参 | 详细设计 D2 |
...
| 4 | P-105 卡片 inline JSON schema 已定义 | ⬜ | §4.3 |
| 4a | OcrSchema 多题契约（v0.1.1 B4 下沉）已定义 | ⬜ | §4.2 |
| 5 | 所有子系统对外接口签名齐全 | ⬜ | §7（S2~S6 全覆盖；S1 在 §6） |
| 6 | LangGraph 编译策略 + checkpointer 配置明确 | ⬜ | §8 |
| 7 | SSE chunk schema 全集已列出（v0.1.1：6 种） | ⬜ | §9.1（含 v0.1.1 新增 `questions_detected` / `sub_problem_changed`）|
| 8 | 错误响应体 + 错误码表齐全 | ⬜ | §5.7 |
| 9 | 关键决策 §10 每条有"备选退路" | ⬜ | §10.2（C1~C16 全覆盖）|
| 10 | 明确不做议题已列出升级触发点 | ⬜ | §11（16 条；其中 C13 联动新增「ProblemType 不单列统计概率」） |
| 11 | 风险表覆盖架构 §12 所有未关闭风险 | ⬜ | §12（含 AR-002/003/005/006/012）|
| 12 | 下游开放问题已分配到详细设计 / 运维 / 流程 | ⬜ | §13.2 |
| 13 | 编写人 + 评审人确认可进入详细设计 | ⬜ | — |
| 14 | B4 下沉路由签名（`subProblemRouter` / `insightLoopRouter`）已定义 | ⬜ | §7.1 / §8.3 / C16 |
| 15 | B4 SubProblemStateSchema 字段（status / insightPoints / per-subProblem 计数）已定义 | ⬜ | §4.1 / C15 |
