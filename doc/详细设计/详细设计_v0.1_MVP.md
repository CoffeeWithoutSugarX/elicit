...
- 每个 Pólya prompt 配 **1~2** 组 few-shot（不超过；超过会显著推高 token 成本）
- few-shot 必须从 PRD §8.4 兜底矩阵 / §8.2 阶段完成判据中提取真实可能场景
- VisionNode / ClassifyNode 不配 few-shot（输入结构高度一致，靠 JSON schema 约束足够）
...
- `visualFeaturesNeeded: boolean` —— 任一题含函数图像 / 统计图表时 true
- `errorReason: "BLURRY" | "NOT_SOLVABLE" | "TIMEOUT" | "PARSE_FAIL" | null` —— **不再包含 `MULTI_QUESTIONS`**（多题不是错误,是正常路径）
...
4. 整张图整体学科判断（若有非数学题混入，subject 取实际学科）。
5. 标注是否含"必须依赖图形形状"的题（函数图像 / 统计图表）。
...
**few-shot**：无（结构化输出靠 JSON schema 约束）。
...
**测试映射**：
- L3 单测：注入 fake model 返回 5 种 fixture（单题可解 / 多题可解 / 非数学 / 模糊 / >5 题截断）→ 断言 `state.ocrResult.questions` 数组 + `isMulti`
- L6 evals 剧本：「出范围」#1（英语题）/「多题处理」#1（2 题选其一）/「正常路径」#1（单题）
...
**测试映射**：
- L3 单测：4 种 fixture（代数 / 几何 / 函数 / 其他）→ 断言 `state.problemType` 写入正确
- L6 evals 剧本：「正常路径」#1~#3（不同题型）

---

#### 4.3.3 UnderstandNode (Pólya ① 理解题意) Prompt — 回答 #1
...
**完成判据**（PRD §8.2 阶段①）：妹妹在回复中**复述出已知条件 + 明确求解目标** → 输出 `phase_signal: "COMPLETED"`。
...
【你的目标】通过反问让妹妹自己确认：题目里的已知条件 + 求解目标她是否真的弄清楚了。
...
【完成判据】当妹妹的回复**复述出题目的已知条件 + 求解目标**（无需 LaTeX 精确，但语义对），输出 phase_signal: "COMPLETED"，并用一句话肯定她"我们对题目的理解一致了，接下来想想从哪入手？"，然后停。
其他情况输出 phase_signal: "STAY"，继续追问没确认到的部分。
...
```ts
userPromptTemplate({ selectedQuestion, recentMessages }) {
  return `# 当前题目
${selectedQuestion.topic}
LaTeX：${selectedQuestion.latexFull}
已知条件：${selectedQuestion.givenConditions.join('; ')}
求解目标：${selectedQuestion.goal}
...
[妹妹]：就是这个方程能解出两个 x 值
[Agent]：对！那"两个"对应到判别式 Δ，应该是 ≥ 0 还是 > 0？想清楚这点，我们就理解了题目。
phase_signal: "STAY"

[妹妹]：Δ ≥ 0 吧，因为可以有两个相等的
[Agent]：很好，那我们已经清楚：已知方程要有两实根（含相等），求 m 的范围。下一步该从哪入手？
phase_signal: "COMPLETED"
```
...
**User Prompt Template**（v0.1.1 B4 下沉：加 `currentSubProblem` + `insightPoints[]` 上下文）：
...
#### 4.3.5 ExecuteNode (Pólya ③ 执行) Prompt — 回答 #1（v0.1.1 B4 下沉：扩 5 信号 + 多破题点）
...
> **信号优先级（互斥）**：DONE > BLOCKED > ESCALATE > STAY；同一轮输出至多一个 `phase_signal`。`new_insight` 与任一信号正交（DONE / STAY 时都可能附带）。
...
#### 4.3.6 ReviewNode (Pólya ④ 回顾) Prompt — 回答 #1 + #17（v0.1.1 B4 下沉：多小问汇总）

**输入字段**：同 §4.3.4 + 知识点术语库（详见 §6.1）+ **所有 subProblems 的 `insightPoints[]` + `status`**（B4 新增）
...
**计数维护**：`state.stuckCountPerPhase[phaseKey]` 在每次 stuckGuard 返回非 null 时由调用节点 +1；妹妹某轮 phase_signal=COMPLETED 时重置。
...
1. 若 state.ocrResult?.errorReason 命中以下集合：
   - "TIMEOUT" / "PARSE_FAIL" / "BLURRY" / "NOT_SOLVABLE"
   → 终止（true）
2. 否则放行（false）
```
...
| 路径 | 大小预估 | 来源 | 备注 |
|---|---|---|---|
| `src/agents/prompts/vision/visionNode.prompt.ts` | ~2.5 KB | §4.3.1 完整 prompt | 含 JSON schema 输出契约；无 few-shot |
| `src/agents/prompts/phases/classifyNode.prompt.ts` | ~1 KB | §4.3.2 | 题型分类规则 + JSON schema |
| `src/agents/prompts/phases/understandNode.prompt.ts` | ~2 KB | §4.3.3 | 含 1 组 few-shot |
...
| 14 | OCR `isSolvable=false`（B4 新增 / P2 边界）| 按 errorReason 分支：<br/>· `BLURRY` → "图片有点模糊，能再拍一张吗？尽量拍正、光线好一点" <br/>· `NOT_SOLVABLE` → "这张图我没识别出数学题，要不你直接把题目打字给我？" <br/>· `TIMEOUT` → "识别有点慢，再试一次？" <br/>· `PARSE_FAIL` → "我没看明白这道题，换一张拍清楚一点的可以吗？" | `vision.unsolvable: ${errorReason}` warn |
...
| E13 | 多小问递进（B4 新增 / FR-AGENT-008）| `multi-subproblems-cascade.eval.ts` | UNDERSTAND→PLAN→EXECUTE(SUB_PROBLEM_DONE)→`subProblemRouter`→UNDERSTAND→PLAN→EXECUTE(SUB_PROBLEM_DONE)→REVIEW→DONE | 二次函数综合题"(1) 求 A、B、P 坐标 (2) 求 △PAB 面积"（subProblems.length=2）▸ 完成小问 (1) SUB_PROBLEM_DONE → SSE 推 `sub_problem_changed {from:0, to:1, total:2}` → 顶栏切到 "小问 2/2" → 小问 (2) 入场时 stuckCountPerPhase / probedQuestionIds 已清零（per-subProblem 隔离）→ 妹妹推出面积 SUB_PROBLEM_DONE → 全部 done → 阶段 ④ 仅 1 张 P-105 卡片含 `subProblemSummaries.length=2` |
| E14 | 部分解卡死（B4 新增 / PRD §8.2 部分解出口 / DR-013）| `multi-subproblems-partial-blocked.eval.ts` | UNDERSTAND→...→EXECUTE(PROBLEM_BLOCKED)→`subProblemRouter`→REVIEW→DONE | 分类讨论题（subProblems.length=1，但难度高）▸ 阶段 ③ 妹妹 5 问探路 + 知识点兜底后仍无突破 → ExecuteNode 输出 `phase_signal: "PROBLEM_BLOCKED"` → `subProblemRouter` 跳过该小问剩余环节进 ④ → 卡片 `subProblemSummaries[0].status='blocked'` + `blockedHint` 含"对 Δ<0 情形分类讨论的辅助技巧" + 推荐看老师讲解；**严禁**给具体数值答案 |
...
_本版本暂无增量补丁。v0.2 / v0.3 大版本升级才新建文件；v0.1.x 小修补按时间倒序追加在此章。_
