...
#### 4.3.3 UnderstandNode (Pólya ① 理解题意) Prompt — 回答 #1（v0.1.1 B4 下沉：per-subProblem 复述 + 多小问感知）
...
**完成判据**（PRD §8.2 阶段① B4 改写：多小问时仅针对当前 subProblem 的条件 + 求解目标）：
- **首个小问**（`currentSubProblemIndex === 0`）：妹妹复述出**当前小问的已知 + 求解目标**（语义对即可，无需 LaTeX 精确）→ COMPLETED
- **后续小问**（`currentSubProblemIndex > 0`）：妹妹**简短复述当前小问目标**（如"现在算 △PAB 面积"）即可 COMPLETED；**不强求她重新复述整道题**——整体题面在首个小问时已理解过

**完整 System Prompt**（B4 多小问感知）：
...
**few-shot 示例**（v0.1.1：2 组覆盖首个小问 + 后续小问简短复述）：
...
[妹妹]：就是这个方程能解出两个 x 值，Δ ≥ 0 吧，因为可以有两个相等的
