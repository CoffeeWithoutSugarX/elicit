...
| D11 | 部署架构：全栈自建到阿里云 ECS | 妹妹用户在贵阳，Vercel + Supabase Cloud 在大陆访问性差；自建 Supabase 全栈到阿里云轻量应用服务器 2C8G + 4M 带宽（¥104/月） | §10.2 / AR-008/009/010（详情已散落落地，本表索引补回）|
| D12 | Supabase 自建栈保留 Kong API Gateway | 沿用 supabase/docker 官方默认部署模式；Nginx（公网入口 + HTTPS 终结 + SSE 反代）+ Kong（Supabase 内部网关，路由 GoTrue/PostgREST + apikey 校验）双层分工，不冗余；不砍 Kong 节省 ~300MB RAM | §10.2 / §17 AR-010 |
...
- **B4 补充（2026-05-21）**：详设 §15 D12 在本节点拓扑上**新增 PLAN ↔ EXECUTE 内层循环条件边 + EXECUTE → UnderstandNode（下一 subProblem）外层推进条件边 + EXECUTE → REVIEW（PROBLEM_BLOCKED 出口）**——属于在"5 个 Pólya 节点 + 守卫"框架内增加条件边的范畴，不引入新节点 / 不拆 subgraph / 不动 D2 结论。VisionNode 集成 Decomposition（详设 §15 D13）同样在 VisionNode 内部演化，不增节点。
...
#### D11 部署架构：全栈自建到阿里云 ECS

> 详情见 §10.2（部署拓扑）+ §17 AR-008（单点故障）/ AR-009（ICP 备案）/ AR-010（Supabase 版本漂移）；本节做决策快照。

- **问题陈述**：妹妹用户在贵阳，主流"Vercel + Supabase Cloud"方案在大陆访问性差（Vercel 主域名被污染 / Supabase Cloud 延迟 200~500ms 偶发断连）。生产部署放哪？
- **候选方案**：
  - A：Vercel + Supabase Cloud 海外原生方案 — 开发体验最佳，但大陆访问不稳定
  - B：阿里云函数计算 + Supabase Cloud — 后端在境内但 DB 仍在境外
  - C：**全栈自建到阿里云 ECS**（Next.js + Supabase 自建栈同台 docker-compose） — 全程国内骨干网
  - D：腾讯云 / 华为云 — 同 C 等价
- **采纳**：**方案 C**（阿里云轻量应用服务器 2C8G + 4M 带宽，¥104 / 月）
- **理由**：
  - 贵阳 → 华东/华南骨干网延迟 < 50ms
  - Next.js 与 Supabase 同台 docker network 内网通信零延迟
  - ¥104/月 远低于 Vercel Pro + Supabase Pro 双订阅（$45+/月）
  - 妹妹 1 + 家长 1 用户场景，2C8G 充裕（详见 §10.2 RAM 估算）
- **备选退路**：用户规模 ≥ 50 或 NFR-BUSINESS 指标卡在单机性能时，升级到 ECS 多可用区 + Postgres 主备 + Nginx 负载均衡（§10.3 future scope）

#### D12 Supabase 自建栈保留 Kong API Gateway

- **问题陈述**：Supabase 官方 `supabase/docker` docker-compose 默认带 Kong 作为内部 API Gateway（路由 GoTrue / PostgREST + apikey 校验）；同时 ECS 主机层已有 Nginx（HTTPS 终结 + SSE 反代）。两层网关共存是否冗余？是否要砍掉 Kong 节省 ~300MB RAM？
- **候选方案**：
  - A：**保留 Kong**（沿用 supabase/docker 默认部署模式） — RAM +200~400MB；Supabase 升级时不破坏；社区文档可搜
  - B：砍掉 Kong，Nginx 直接代理 GoTrue/PostgREST — 省 ~300MB RAM；但 Nginx 要手写 anon/service_role key 校验逻辑；每次 Supabase 升级要对齐 GoTrue/PostgREST 内部 header 规约
  - C：用阿里云 API Gateway 替代 Kong — 多一个云服务费用 + 跨网络往返延迟
- **采纳**：**方案 A**（保留 Kong；§10.2 拓扑图 + 详设 §7.4 已落地）
- **理由**：
  - 300MB 在 8GB ECS 上不是瓶颈：Postgres + Next.js + GoTrue + PostgREST + Kong 总 3~4GB，留 4GB 给 OS / docker / 缓冲
  - 沿用 Supabase 官方部署模式 → 升级零破坏 + 出问题能在社区搜到答案
  - Nginx + Kong **不冗余而是各管一段**：Nginx 公网入口（HTTPS / SSE 反代 / ICP 备案域名）+ Kong Supabase 内部路由（GoTrue auth / PostgREST REST + RLS）+ apikey 校验
  - 详设 §7.4 已精简 Kong 配置（关闭 Realtime / Storage / Inbucket 路由，仅保留 GoTrue + PostgREST + Studio）
- **备选退路**：当 RAM 紧（如 v0.2 引入新业务后 RAM 总占用 > 7GB 持续）时，评估方案 B 砍 Kong；详设期 §12.X 部署 Runbook 需明确"精简 Kong 路由清单 + 关闭的 Supabase 子服务清单"以减少升级时的对齐工作量
...
| **AR-012** | **双层循环复杂度**（B4 引入）：state 维度从 conversation 级扩到 `(conversation, subProblem)` 级 —— `subProblems[]` / `currentSubProblemIndex` / 每个 subProblem 独立 `insightPoints[]` / `stuckCountPerPhase` 按 `(subProblem, phase)` 隔离；多层条件边（PLAN ↔ EXECUTE 内层 + SubProblem 推进外层 + PROBLEM_BLOCKED 出口）使调试 / 漂移检测难度上升；checkpoint 体积变大 | S5 / D2（B4 补充）/ FR-AGENT-008/009 | 中 | ① 概要 §4 强 Zod 全字段类型守门 + per-subProblem 命名约定（详设 §15 D12 选定扁平 vs 嵌套）；② 详设 §14 L3 单测覆盖 SubProblemRouter / InsightLoopRouter / StuckGuard per-subProblem 隔离；③ 详设 §14.4 L6 evals E12/E13/E14 行为评测；④ 实施期监测 checkpoint 大小，若 P95 > 100KB 评估 `insightPoints` 持久化形态调整 | 详细设计 D12 + 实施期 Phase 2/3 验证 |
