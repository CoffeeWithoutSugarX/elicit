...
export const GET = withAuth(async (_request, { params, user }) => {
    ...
    try {
        ...
        if (!values || values.currentPhase === undefined) {
            return NextResponse.json(BaseResponse.ofSuccess({
                currentPhase: 0,
        ...
        if (!values.userId || values.userId !== user.id) {
            return NextResponse.json(BaseResponse.ofError('无权访问该会话'), { status: 403 });
        }
        ...
        const message = err instanceof Error ? err.message : '获取会话状态失败，请重试';
        return NextResponse.json(BaseResponse.ofError(message), { status: 500 });
    }
});
